...
router.post("/checkout", authMiddleware, async (req: AuthRequest, res: Response) => {
  ...
  if (!priceId) {
    return res.status(500).json({ error: "ID de prix Stripe non configuré" });
  }

  try {
    const db = getPool();
    const [rows] = await db.query("SELECT * FROM cg_users WHERE id=?", [req.user!.userId]);
    ...
    if (!customerId) {
      ...
      await db.query("UPDATE cg_users SET stripe_customer_id=? WHERE id=?", [customerId, req.user!.userId]);
    }
...
router.post("/portal", authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    const db = getPool();
    const [rows] = await db.query("SELECT stripe_customer_id FROM cg_users WHERE id=?", [req.user!.userId]);
    const user = (rows as any[])[0];
...
router.post("/webhook", async (req: Request, res: Response) => {
  ...
  switch (event.type) {
    case "checkout.session.completed": {
      ...
      if (userId && plan) {
        await db.query(
          "UPDATE cg_users SET plan=?, subscription_status='active', stripe_subscription_id=? WHERE id=?",
          [plan, session.subscription, parseInt(userId)]
        );
        console.log(`[Stripe] ✅ Abonnement ${plan} activé pour user ${userId}`);
      }
      break;
    }
    case "customer.subscription.deleted": {
      ...
      await db.query(
        "UPDATE cg_users SET plan='free', subscription_status='inactive' WHERE stripe_subscription_id=?",
        [sub.id]
      );
      console.log(`[Stripe] ⚠️ Abonnement annulé: ${sub.id}`);
      break;
    }
    case "invoice.payment_failed": {
      const inv = event.data.object as Stripe.Invoice;
      await db.query(
        "UPDATE cg_users SET subscription_status='inactive' WHERE stripe_customer_id=?",
        [(inv as any).customer]
      );
      break;
    }
